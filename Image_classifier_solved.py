...
class ClasifierNet(nn.Module):
    def __init__(self, nr_classes : int = 2):
        ...
        # trains in about 16000 steps
        # self.conv_layers = nn.Sequential(
        #                         nn.Conv2d(3, 16, 3, padding=1, stride = 2), nn.ReLU(),
        #                         nn.Conv2d(16, 32, 3, padding=1, stride = 2), nn.ReLU(),
        #                         nn.Conv2d(32, 32, 3, padding=1, stride = 1), nn.ReLU(),
        #                         nn.Conv2d(32, nr_classes, 3, padding=1))

        # trains in about 7000 steps
        ...
        self.conv_layers = nn.Sequential(
                                ...
                                nn.Conv2d(64, 32, 3, padding=1, stride = 1), nn.ReLU(),
                                nn.Conv2d(32, self.nr_classes, 3, padding=1))
        self.logits = torch.nn.AdaptiveAvgPool2d(1)
        self.compute_probs = torch.nn.Softmax(dim = 1) # compute softmax along the channels axis
    ...
    def forward(self, x):
        ...
        class_probabilities = self.compute_probs(logits)
